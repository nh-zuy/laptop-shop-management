package laptopshop.entities.border;
...
public class RadiusBorder extends AbstractBorder {
	...
	public RadiusBorder(Color color) {
		this(color, 4, 8, 7);
	}
	
	public RadiusBorder(Color color, int thickness, int radii, int pointerSize) {
	...
	public RadiusBorder(Color color, int thickness, int radii, int pointerSize, boolean left) {
	...
	public void paintBorder(Component c, Graphics g, int x, int y, int width, int height) {
		...
		if (left) {
			...
			// right point
			pointer.addPoint(width - (strokePad + radius + pointerPad + pointerSize), bottomLineY);
			// bottom point
			pointer.addPoint(width - (strokePad + radius + pointerPad + (pointerSize / 2)), height - strokePad);
		}
